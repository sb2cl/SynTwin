function derivative_time_series = Time_Derivative(Input_time_series,sampling_time,wn)
...
% Input_time_series is the sampled data 
% Sampling is assumed to be in minutes
% derivative_time_series the estimate of the derivative in mins^{-1}
% wn is used to get the number of data points used for smoothing. It corresponds to the fraction of data points used 
...
der_temp_data_clean = smoothdata(der_temp_data,"rlowess",8);
% We use the smoother:
time_series_samples = 0:sampling_time:sampling_time*(length(der_temp_data_clean)-1);
smoother_window = floor(length(time_series_samples)*wn);
derivative_time_series = smoothdata(der_temp_data_clean,'gaussian',smoother_window,'SamplePoints',time_series_samples);
end
