function Plot_Results_Lib5_reduced_model(Results_Lib,Lower_Bounds,Upper_Bounds,indices_plasmids,indices_promoters,indices_rbss,title_text,varargin)
%% Plot_Results_Lib5_reduced_model
% Generate publication-oriented plots for the Lib5 reduced model.
%
% The function produces:
%   - the J23100 Omega estimate distribution;
%   - a one-by-five tiled comparison of experimental and predicted synthesis
%     rates for the five RBS contexts.
%
...
%
% The expected Lib5 tensor dimensions are 1-by-1-by-5.
...
p.addParameter('FigurePrefix', 'Lib5_ALL_reduced', @(x)ischar(x)||isstring(x));
...
% -------------------------------------------------------------------------
% 1) Promoter strength histograms
% -------------------------------------------------------------------------
fig1 = figure('Name','Lib5 promoter strength histograms','Color','w', ...
    'Units','centimeters','Position',[1 1 opt.FigPromoterSizeCm]);
tl1 = tiledlayout(fig1,1,num_promoters,'Padding','compact','TileSpacing','compact'); %#ok<NASGU>
...
save_figure_if_requested(fig1,opt.OutputDir,[opt.FigurePrefix '_hist_promoters'],opt.SaveFigures,opt.ExportPause);
...
% -------------------------------------------------------------------------
fig2 = figure('Name','Lib5 experimental vs predicted Pi(mu) tiles','Color','w', ...
    'Units','centimeters','Position',[1 1 opt.FigTilesSizeCm]);
tl2 = tiledlayout(fig2,1,num_rbss, ...
...
end % function Plot_Results_Lib5_reduced_model
...
% ========================================================================
function code = l5_code(i,j,k)
%L5_CODE Return the construct code for library L5.
%
% Lib5 contains one pGreen/J23100 context and five RBS variants.
    if i ~= 1 || j ~= 1
        error('Plot_Results_Lib5_reduced_model:InvalidTensorIndex', ...
            'Expected Lib5 tensor indices i=1 and j=1.');
    end
    idx = k;
    code = sprintf('L5_%02d', idx);
end
