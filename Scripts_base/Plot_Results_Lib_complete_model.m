function Plot_Results_Lib_complete_model(Results_Lib,Lower_Bounds,Upper_Bounds,indices_plasmids,indices_promoters,indices_rbss, title_text)
% The function takes the complete results of the estimation of a library,
% as stord in the files generated by Generate_Results_Lib30_L1O_full_model.m or , Generate_Results_Lib30_ALL_full_model.m and plots the main features.
%
% Notice if you just want to plot experimental results, you can use Plot_ExpData_Tensor_v1.m
...
% HISTOGRAMS OF PLASMIDS
figure();
tiledlayout(1,1);
for k=1:1
    nexttile;
    Data_plasmid_cn_mean = Results_Lib{k,1,1}.Gene_cn_mean;
    Data_plasmid_cn_raw = Results_Lib{k,1,1}.Gene_cn_raw;
    %pd1 = histfit(Data_plasmid_cn_raw,[],'kernel','Kernel','epanechnikov');%nbins set to default round(sqrt(num_data))
    pd1 = histfit(Data_plasmid_cn_raw,[],'normal');%nbins set to default round(sqrt(num_data))
    grid on;
    pd1(1).FaceColor = color_grey;
    pd1(2).Color = color_grey;
    pd1(1).FaceAlpha = 1;
    hold on
    
    xline( Data_plasmid_cn_mean,'Color','k','LineWidth',1 ); 
    xlabel('Gene copy number','FontSize',18,'Interpreter','latex')
    t= title([title_text,'Plasmid ', Results_Lib{k,1,1}.TU_Ori]);
    t.FontSize = 18;
    ax=gca;
...
% HISTOGRAMS OF PROMOTERS
figure();
tiledlayout(1,num_promoters);
for k=1:num_promoters
    nexttile;
    Data_omega_mean = Results_Lib{1,k,1}.Omega_mean;
    Data_omega_raw = Results_Lib{1,k,1}.Omega_raw;
    pd1 = histfit(Data_omega_raw,[],'normal');%nbins set to default round(sqrt(num_data))
    %pd1 = histfit(Data_omega_raw,[],'kernel','Kernel','epanechnikov');%nbins set to default round(sqrt(num_data))
    grid on;
    pd1(1).FaceColor = Promoter_colors(k,:);
    pd1(2).Color = Promoter_colors(k,:);
    pd1(1).FaceAlpha = 1;
    hold on
    xline( Data_omega_mean,'Color','k','LineWidth',1 ); 
    xlabel('$\omega\, (\mathrm{molec}\cdot\mathrm{min}^{-1})$','FontSize',18,'Interpreter','latex')
    if k==1
      t= title([title_text, Results_Lib{1,k,1}.TU_Promoter]);
    else
       t= title([Results_Lib{1,k,1}.TU_Promoter]);
    end
    t.FontSize = 18;
    ax=gca;
    ax.XAxis.FontSize = 18;
...
for k=1:num_rbss
    ...
    xlabel('$\kappa^0\, (\mathrm{molec}^{-1})$','FontSize',18,'Interpreter','latex')
    if k==1
...
for k=1:num_rbss
    nexttile;
    Data_rbs_inv_sigma0_mean = Results_Lib{1,1,k}.RBS_inv_sigma0_mean;
    Data_rbs_inv_sigma0_raw = Results_Lib{1,1,k}.RBS_inv_sigma0_raw;
    pd1 = histfit(Data_rbs_inv_sigma0_raw,[],'normal');%nbins set to default round(sqrt(num_data))
    %pd1 = histfit(Data_omega_raw,[],'kernel','Kernel','epanechnikov');%nbins set to default round(sqrt(num_data))
    grid on;
    pd1(1).FaceColor = RBS_colors(k,:);
    pd1(2).Color = RBS_colors(k,:);
    pd1(1).FaceAlpha = 1;
    hold on
    xline( Data_rbs_inv_sigma0_mean,'Color','k','LineWidth',1 ); 
    xlabel('$\rho^0\, (\mathrm{adim})$','FontSize',18,'Interpreter','latex')
     t.FontSize = 18;
    ax=gca;
    ax.XAxis.FontSize = 18;
    ax.YAxis.FontSize = 18;
...
% HISTOGRAMS OF R^BSs k0, sigma0 and crosscorrelation
figure();
tiledlayout(3,num_rbss);
% First the k's
for k=1:num_rbss
    nexttile;
    Data_rbs_k0_sigma0_mean = Results_Lib{1,1,k}.RBS_k0_sigma0_mean;
...
for k=1:num_rbss
    ...
    Data_rbs_inv_sigma0_raw = Results_Lib{1,1,k}.RBS_inv_sigma0_raw;
    grid on;
    scatter(Data_rbs_inv_sigma0_raw,Data_rbs_k0_sigma0_raw,5,'MarkerEdgeColor',RBS_colors(k,:),...
              'MarkerFaceColor',RBS_colors(k,:),'SizeData',50)

    hold on
    yline( Data_rbs_k0_sigma0_mean,'Color','k','LineWidth',1 ); 
    xline( Data_rbs_inv_sigma0_mean,'Color','k','LineWidth',1 ); 
    xlabel('$\rho^0\, (\mathrm{adim})$','FontSize',18,'Interpreter','latex')
    ylabel('$\kappa^0\, (\mathrm{molec}^{-1})$','FontSize',18,'Interpreter','latex')
     t.FontSize = 18;
    ax=gca;
    ax.XAxis.FontSize = 18;
    ax.YAxis.FontSize = 18;
...
% HISTOGRAMS OF R^BSs kappa and correlation kappa-rho
figure();
tiledlayout(2,num_rbss);
% First the k's
for k=1:num_rbss
...
for k=1:num_rbss
    nexttile;
    Data_rbs_k0_mean = Results_Lib{1,1,k}.RBS_k0_sigma0_mean./Results_Lib{1,1,k}.RBS_inv_sigma0_mean;
    Data_rbs_sigma0_mean = 1./Results_Lib{1,1,k}.RBS_inv_sigma0_mean;
    Data_rbs_k0_raw = Results_Lib{1,1,k}.RBS_k0_sigma0_raw./Results_Lib{1,1,k}.RBS_inv_sigma0_raw;
    Data_rbs_sigma0_raw = 1./Results_Lib{1,1,k}.RBS_inv_sigma0_raw;
    grid on;
    scatter(Data_rbs_sigma0_raw,Data_rbs_k0_raw,5,'MarkerEdgeColor',RBS_colors(k,:),...
              'MarkerFaceColor',RBS_colors(k,:),'SizeData',50)

    hold on
    yline( Data_rbs_k0_mean,'Color','k','LineWidth',1 ); 
    xline( Data_rbs_sigma0_mean,'Color','k','LineWidth',1 ); 
    xlabel('$\sigma^0\, (\mathrm{adim})$','FontSize',18,'Interpreter','latex')
    ylabel('$k^0\, (\mathrm{molec}^{-1})$','FontSize',18,'Interpreter','latex')
     t.FontSize = 18;
    ax=gca;
    ax.XAxis.FontSize = 18;
    ax.YAxis.FontSize = 18;
...
figure();
tiledlayout(num_rbss,num_promoters*num_plasmids);
          ...
          if not(isempty(Results_Lib{i,j,k}))
           ...
           % Full set of experimental data 
            ...
            for q=1:num_instances
              % no_wells = length(Results_Lib{i,j,k}.Instances{q}.Wells);
              %   for r=1:no_wells
              %       y = Results_Lib{i,j,k}.Exp_Data.Instances{q}.Wells(r). Pi_mumax_pmax;
              %       x = Results_Lib{i,j,k}.Exp_Data.Instances{q}.Wells(r). Mu_mumax_pmax;
              %       s= plot(x,y,'Color',color_grey,'LineStyle','-','LineWidth',0.2,'HandleVisibility','off');
              %       hold on
              %  end %no_wells
