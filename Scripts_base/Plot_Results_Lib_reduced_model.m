function Plot_Results_Lib_reduced_model(Results_Lib,Lower_Bounds,Upper_Bounds,indices_plasmids,indices_promoters,indices_rbss, title_text)
% The function takes the complete results of the estimation of a library,
% as stored in the files generated by Generate_Results_LibX_L1O_approx_model.m and
% Generate_Results_LibX_ALL_approx_model.m with X=24,30, and plots the main features.
%
...
for k=1:1
    ...
    pd1(1).FaceAlpha = 1;
    hold on
    % xline( x_L(end)*model_c.N_pSC101,'Color','r','LineWidth',2 );
    % xline( x_U(end)*model_c.N_pSC101,'Color','r','LineWidth',2 );
    xline( Data_plasmid_cn_mean,'Color','k','LineWidth',1 ); 
    xlabel('Gene copy number','FontSize',22,'Interpreter','latex')
    t= title([title_text,'Histogram of Estimated Parameters for Plasmid ', Results_Lib{k,1,1}.TU_Ori]);
    t.FontSize = 16;
    ax=gca;
...
for k=1:num_promoters
    ...
    pd1(1).FaceAlpha = 1;
    hold on
    % xline( x_L(k),'Color','r','LineWidth',2 );
    % xline( x_U(k),'Color','r','LineWidth',2 );
    xline( Data_omega_mean,'Color','k','LineWidth',1 ); 
    xlabel('$\omega\, (\mathrm{molec}\cdot\mathrm{min}^{-1})$','FontSize',22,'Interpreter','latex')
    if k==1
...
for k=1:num_rbss
    ...
    xlabel('$\frac{k_0}{\sigma_0}\, (\mathrm{molec}^{-1})$','FontSize',18,'Interpreter','latex')
    if k==1
