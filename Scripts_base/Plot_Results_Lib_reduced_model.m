function Plot_Results_Lib_reduced_model(Results_Lib,Lower_Bounds,Upper_Bounds,indices_plasmids,indices_promoters,indices_rbss,title_text,varargin)
% Plot_Results_Lib_reduced_model_v9
%
% Publication-oriented plots for Lib24 L1O reduced-model results.
%
% Generates and optionally exports four main figures:
%   1) histogram of estimated pGreen effective copy number
%   2) histograms of estimated promoter strengths
%   3) histograms of estimated RBS intrinsic initiation capacities kappa^0
%   4) tiled experimental-vs-predicted Pi(mu) plots for L24 constructs
%
% Color style:
%   HEX codes are converted explicitly to RGB using hex2rgb_local().
%   RBS colors are mapped through indices_rbss, so the L24 RBS subset
%   [B0030, B0032, J61100, J61101] uses the canonical colors defined below
%   without accidentally assigning the B0034 color to J61100.
%
% Font tuning:
%   Use name-value arguments such as:
%       'BaseFontSize',26,'TileFontSize',18,'AxisLabelFontSize',24
%   Histograms use BaseFontSize. The Pi-vs-mu tiled figure uses
%   TileFontSize for tick labels and AxisLabelFontSize for axis labels.
%
% Aspect-ratio / figure-size tuning:
%   All figure windows are created in centimeters before export. The exported
%   PNG/SVG inherits that geometry. To make a figure taller or wider without
%   touching the plotting code, change these name-value arguments:
%       'FigCopySizeCm',     [28 22]   % [width height], pGreen copy number
%       'FigPromoterSizeCm', [72 24]   % promoter histograms
%       'FigRBSSizeCm',      [84 24]   % RBS histograms
%       'FigTilesSizeCm',    [76 48]   % Pi-vs-mu tiled figure
%   Examples:
%       % More vertical space for the tiled plot:
%       'FigTilesSizeCm',[76 55]
%       % Less vertically stretched histograms:
%       'FigPromoterSizeCm',[72 18]
%       % Wider tiled plot for Supplementary landscape page:
%       'FigTilesSizeCm',[90 45]
%
% Example:
%   Plot_Results_Lib_reduced_model_v9(R,LB,UB,[1 2],[1 2 3],[1 2 4 5],'', ...
...
p.addParameter('FigurePrefix', 'Lib24_L1O_reduced', @(x)ischar(x)||isstring(x));
...
% -------------------------------------------------------------------------
fig1 = figure('Name','Lib24 pGreen copy number histogram','Color','w', ...
...
% -------------------------------------------------------------------------
fig2 = figure('Name','Lib24 promoter strength histograms','Color','w', ...
...
% -------------------------------------------------------------------------
fig3 = figure('Name','Lib24 RBS kappa histograms','Color','w', ...
    'Units','centimeters','Position',[1 1 opt.FigRBSSizeCm]);
tl3 = tiledlayout(fig3,1,num_rbss,'Padding','compact','TileSpacing','compact'); %#ok<NASGU>
for k = 1:num_rbss
    ax = nexttile;
    hold(ax,'on');
...
save_figure_if_requested(fig3,opt.OutputDir,[opt.FigurePrefix '_hist_RBS_kappa'],opt.SaveFigures,opt.ExportPause);

% -------------------------------------------------------------------------
% 4) Tiled experimental vs predicted Pi(mu)
% -------------------------------------------------------------------------
fig4 = figure('Name','Lib24 experimental vs predicted Pi(mu) tiles','Color','w', ...
    'Units','centimeters','Position',[1 1 opt.FigTilesSizeCm]);
tl4 = tiledlayout(fig4,num_rbss,num_promoters*num_plasmids, ...
...
save_figure_if_requested(fig4,opt.OutputDir,[opt.FigurePrefix '_Pi_vs_mu_tiles'],opt.SaveFigures,opt.ExportPause);

end % function Plot_Results_Lib_reduced_model_v9
...
% ========================================================================
function code = l24_code(i,j,k)
%L24_CODE Return EC code for library L24.
%
% INTERNAL tensor convention:
%   i = 1 -> pGreen
%   i = 2 -> pSC101
%
% Official L24/Table S1 convention:
%   L24_01--L24_12 -> pSC101
%   L24_13--L24_24 -> pGreen
