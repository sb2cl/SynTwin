...
% SynTwin workflow script: post-processing and compilation of L6 (L1O)
...
%
%   In this workflow:
%     - The shared RBS parameter k0_sigma0 is estimated under L1O cross-validation.
%     - Ori-/promoter-dependent parameters (e.g., Omega and Gene copy number)
%       are inherited from Lib24 inference results.
%     - Experimental measurements for the corresponding 6 TUs are retrieved by
%       indexing the Lib30-format experimental tensor container.
%
% DESCRIPTION
%   Loads the per-fold optimization outputs produced by
%   Estimate_Lib6_L1O_reduced_model (one file per left-out construct) and compiles
%   them into a unified Results tensor for the L6 sublibrary.
%
%   For each L1O fold (i.e., each left-out construct), the script:
%     1) Loads the estimated k0_sigma0 samples for that fold.
%     2) Stores fold-specific ("local") statistics in:
%          Parameters_local_raw / mean / std
%        which correspond to the parameter inferred when that TU is left out.
%
%   In addition, the script concatenates all k0_sigma0 samples across folds to
%   compute global L6-level ("ALL") statistics:
%       RBS_k0_sigma0_raw / mean / std
%
%   The output tensor also includes:
%     - inherited Lib24 parameter statistics (Omega, Gene_cn),
%     - experimental Mu(t) and Pi(t) trajectories (Global / Instances / Wells),
%     - digital-twin predictions over experimental Mu(t),
%     - local relative sensitivities mapped to absolute sensitivities,
...
%     Example: Results_Tensor_Lib6_L1O_reduced_model_Wells.mat
%
...
%   - Experimental tensor container (L6 is extracted by indexing):
%       Experimental_Data/ExpData_Tensor_lib30_micro.mat
%   - Inherited Lib24 results (Omega and Gene_cn distributions/statistics):
...
%     Expected naming pattern (one file per left-out construct):
...
%     Parameters_local_* fields; global RBS statistics are also reported by
%     pooling all folds (RBS_k0_sigma0_*).
%   - Monte Carlo predictions propagate uncertainty from both inherited (Lib24)
%     and estimated (L6) parameters.
...
for p=1:length(indices_plasmids_lib6)
    for q=1:length(indices_promoters_lib6)
        for r=1:length(indices_rbss_lib6)
            ...
            load(file_tensor, "Results_BADS_B0034_LOOCV_approx", "-mat");

            num_runs = length(Results_BADS_B0034_LOOCV_approx);
            tempo = [];
            for i=1:num_runs
               tempo=[tempo;Results_BADS_B0034_LOOCV_approx{i}.results(:,1)];
               Matrix_tempo_All=[Matrix_tempo_All;Results_BADS_B0034_LOOCV_approx{i}.results(:,1)];
            end
...
save(file_tensor, "Results_Tensor_Lib6_L1O_reduced", "-mat");
