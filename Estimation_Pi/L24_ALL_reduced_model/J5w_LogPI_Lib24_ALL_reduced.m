...
%
% COST FUNCTION
%   Parameter estimation is performed in base-10 logarithmic space using
%   a pseudo-Huber loss applied to the residuals between predicted and
%   experimental synthesis rates:
%
%       r = log10(predicted_Pi_matrix) - log10(Pi_exp_matrix)
%
%       rho(r) = delta^2 * ( sqrt(1 + (r./delta).^2) - 1 )
%
%   To mildly prioritize datapoints with higher synthesis rates, a smooth
%   magnitude-dependent weighting is introduced:
%
%       weight = sqrt( 1 + log10( 1 + Pi_exp_matrix ) )
%
%   The weighted pseudo-Huber loss is therefore
%
%       weight .* rho(r)
%
%   This formulation preserves scale invariance across synthesis-rate
%   regimes while providing robustness to outliers. The additional
%   weighting slightly emphasizes higher-synthesis regimes, which typically
%   exhibit higher signal-to-noise ratios and are therefore more informative
%   for parameter estimation.
%
%   The scalar objective J is obtained by averaging the weighted loss
%   across all TUs, growth-rate conditions, and replicates.
%
% INPUTS
%   params:       Parameter vector to be estimated. 
%
...
%
%   Remaining inputs provide the circuit model, aggregation layer,
%   experimental tensor, HEM surrogate, fixed rho^0, and pseudo-Huber scale.
%
%       - model_c:
...
%
% OUTPUTS
%   Sum_prediction_error: Scalar objective value (lower is better).
%
% NOTES
