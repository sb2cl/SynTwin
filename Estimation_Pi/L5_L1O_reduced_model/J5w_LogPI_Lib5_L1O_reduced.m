function Sum_prediction_error = J5w_LogPI_Lib5_L1O_reduced(params,Construct_2_LO,model_c,Use_mean,ExpData_Tensor_lib5_micro,Inherited_ParamsData,HEM,num_run,RBS_inv_sigma0,delta)
%% J5w_LogPI_Lib5_L1O_reduced
% Evaluate one Lib5 leave-one-out reduced-model objective.
%
% DESCRIPTION
%   Excludes the RBS-defined construct indexed by Construct_2_LO and computes
%   the weighted pseudo-Huber objective over the remaining four constructs.
%
%   The fitted scalar is the J23100 promoter parameter, Omega. Effective copy
%   number and RBS kappa^0 values are inherited from the Monte Carlo sample
%   indexed by num_run.
%
% OUTPUT
...
num_constructs = length(indices_rbss) - 1;
...
for k=1:length(indices_rbss)
    if  ~( k==Construct_2_LO )% Include all constructs except the selected left-out fold.
        Omega = params;
