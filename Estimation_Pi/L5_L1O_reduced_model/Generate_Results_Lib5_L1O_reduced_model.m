...
% Compile the Lib5 leave-one-out reduced-model result tensor.
%
% DESCRIPTION
%   Loads the five fold-specific J23100 estimation files and builds a
%   1-by-1-by-5 result tensor containing:
%
%   - local Omega distributions for each left-out construct;
%   - pooled Omega distributions across all folds;
%   - local and pooled objective-function values;
%   - inherited pGreen and RBS parameter distributions;
...
%
% OUTPUT
%   Results_Tensor_Lib5_L1O_reduced_<Use_mean>.mat
%
...
for p=1:length(indices_plasmids_lib5)
    for q=1:length(indices_promoters_lib5)
        for r=1:length(indices_rbss_lib5)
            ...
            S = load(file_tensor, "Results_BADS_J23100_L1O_reduced");
            if ~isfield(S,'Results_BADS_J23100_L1O_reduced')
                error('Generate_Results_Lib5_L1O_reduced_model:MissingVariable', ...
                    'Expected Results_BADS_J23100_L1O_reduced in %s.', file_tensor);
            end
            Results_BADS_J23100_L1O_reduced = S.Results_BADS_J23100_L1O_reduced;
            num_runs = length(Results_BADS_J23100_L1O_reduced);
            omega_local = [];
            J_local = [];
            for run_idx = 1:num_runs
                run_results = Results_BADS_J23100_L1O_reduced{run_idx}.results;
                omega_local = [omega_local; run_results(:,1)]; %#ok<AGROW>
                J_local = [J_local; run_results(:,2)]; %#ok<AGROW>
                Matrix_tempo_All = [Matrix_tempo_All; run_results]; %#ok<AGROW>
            end
            Estimated_parameters_J23100.TU{p,q,r}.raw = omega_local;
            Estimated_parameters_J23100.TU{p,q,r}.mean = mean(omega_local,1);
            Estimated_parameters_J23100.TU{p,q,r}.std = std(omega_local,0,1);
            Estimated_parameters_J23100.TU{p,q,r}.J_raw = J_local;
        end %rbss
    end %promoters
end %plasmids
Estimated_parameters_J23100.ALL_raw = Matrix_tempo_All(:,1);
Estimated_parameters_J23100.ALL_mean = mean(Estimated_parameters_J23100.ALL_raw,1);
Estimated_parameters_J23100.ALL_std = std(Estimated_parameters_J23100.ALL_raw,0,1);
Estimated_parameters_J23100.J_raw = Matrix_tempo_All(:,2);

A=-1;
B=-1e-6;
n_samples_MC = min([1000, ...
    numel(Inherited_ParamsData.Gene_cn_MC_samples), ...
    cellfun(@(x)numel(x.RBS_k0_sigma0_MC_samples),Inherited_ParamsData.RBS)]);
...
% Build the Lib5 leave-one-out result tensor.
for r=1:num_rbss_Lib5
  ...
  Results_Tensor_Lib5_L1O_reduced{1,1,r}.TU_color_code =  ExpData_Tensor_lib5_micro{1,1,indices_rbss_lib5(r)}.TU_color_code;
   ...
   %Local estimations are the estimated parameters for all TUs but
    ...
    Results_Tensor_Lib5_L1O_reduced{1,1,r}.J_local_raw = Estimated_parameters_J23100.TU{1,1,r}.J_raw;
    
   %Estimated parameters (Ori and RBSs inherited):
    Results_Tensor_Lib5_L1O_reduced{1,1,r}.Gene_cn_raw = Results_Tensor_Lib24_L1O_reduced{1,1,1}.Gene_cn_raw;
    Results_Tensor_Lib5_L1O_reduced{1,1,r}.Gene_cn_mean = Results_Tensor_Lib24_L1O_reduced{1,1,1}.Gene_cn_mean;
    Results_Tensor_Lib5_L1O_reduced{1,1,r}.Gene_cn_std =  Results_Tensor_Lib24_L1O_reduced{1,1,1}.Gene_cn_std;
...
save(file_tensor, "Results_Tensor_Lib5_L1O_reduced", "Estimated_parameters_J23100", "-mat");