%% Show_Results_Lib5_ALL_reduced_model
% Visualize the distributed Lib5 all-construct reduced-model results.
%
% Loads Results_Tensor_Lib5_ALL_reduced_Wells.mat and calls the shared
% Scripts_base/Plot_Results_Lib5_reduced_model.m function.
%
% USAGE
%   Show_Results_Lib5_ALL_reduced_model
%
% See README.md for the complete workflow.

% --- Portable project initialization (no absolute paths) ---
ROOT = init_SynTwin();  % Scripts_base + HEM_Surrogate are added by default
% Absolute path to the folder where this script lives (model folder)
SCRIPT_DIR = fileparts(mfilename('fullpath'));
addpath(SCRIPT_DIR);

OUTPUT_DIR = fullfile(SCRIPT_DIR,'Figures');
if ~exist(OUTPUT_DIR,'dir')
    mkdir(OUTPUT_DIR);
end

results_file = fullfile(SCRIPT_DIR,'Results_Tensor_Lib5_ALL_reduced_Wells.mat');
S = load(results_file);
if ~isfield(S,'Results_Tensor_Lib5_ALL_reduced')
    error('Show_Results_Lib5_ALL_reduced_model:MissingVariable', ...
        'Expected variable Results_Tensor_Lib5_ALL_reduced in %s.', results_file);
end
Results_Tensor_Lib5_ALL_reduced = S.Results_Tensor_Lib5_ALL_reduced;

% Consider an exogenous circuit. We use a Transcriptional Unit (TU) expressing GFP:
model_c.lp_c = 240; %Length of GFP protein (aa)     
model_c.le_c = model_c.lp_c^0.097/0.0703; 	%Ribosome occupancy length (aa) %lea=la^0.097/0.0703
model_c.dm_c  =  0.2; %Mean degradation rate of non-ribosomal mRNA (1/min)
model_c.Em_c =  model_c.lp_c/model_c.le_c*(1- (model_c.lp_c/(model_c.lp_c+model_c.le_c))^(model_c.lp_c/model_c.le_c)) ;  
model_c.WEm_c =  1 + 1/model_c.Em_c;  
model_c.N_pSC101 = 5; %known

indices_plasmids = 1;
num_plasmids = length(indices_plasmids);
indices_promoters = 4;
num_promoters = length(indices_promoters);
indices_rbss = [1,2,3,4,5];
num_rbss = length(indices_rbss);

 Lower_Bounds = 0.02;   %lower expected bounds   Promoter Omega,     J23100          
 Upper_Bounds = 1.5;  %upper expected bounds  Promoter Omega,     J23100    

 title_text = '';

  Plot_Results_Lib5_reduced_model(Results_Tensor_Lib5_ALL_reduced, ...
        Lower_Bounds,Upper_Bounds,indices_plasmids,indices_promoters,indices_rbss,title_text, ...
        'OutputDir',OUTPUT_DIR, ...
        'FigurePrefix','Lib5_ALL_reduced_Wells', ...
        'SaveFigures',true, ...
        'BaseFontSize',26, ...
        'TileFontSize',18, ...
        'AxisLabelFontSize',24,...
        'FigPromoterSizeCm',[45 30],...
        'FigTilesSizeCm',[75 25]);


