function Sum_prediction_error = J5w_LogPI_Lib24_L1O_reduced(params,Construct_2_LO,model_c,Use_mean,ExpData_Tensor_lib30_micro,HEM,RBS_inv_sigma0,delta)
%% J5w_LogPI_Lib24_L1O_reduced
% Evaluate one Lib24 leave-one-out reduced-model objective.
%
% DESCRIPTION
%   Excludes the construct identified by Construct_2_LO and computes the
%   weighted pseudo-Huber objective over the remaining 23 constructs.
%
% PARAMETERS
%   The parameter vector contains three promoter rates, four RBS intrinsic
%   initiation capacities, and one pGreen/pSC101 copy-number multiplier.
%   The RBS inverse scaling parameter rho^0 is fixed.
%
% COST FUNCTION
%   Residuals are evaluated in base-10 logarithmic synthesis-rate space:
%
%       r = log10(Pi_pred) - log10(Pi_exp)
%
%   and penalized with a weighted pseudo-Huber loss. The objective is averaged
%   over growth-rate points, replicates, and the 23 training constructs.
%
% OUTPUT
            ...
            if  ~( (i==Construct_2_LO(1,1))&&(j==Construct_2_LO(1,2))&&(k==Construct_2_LO(1,3)) ) % If construct to be considered    
                ...
                % obtain the cost index using the mean data over the instances of the construct
                    ...
                    if any(isnan(Pi_predictions))
                        ...
                        w = sqrt(1 + log10(1 + Pi_experimental));
                        Sum_prediction_error =  Sum_prediction_error +  sum(w.*rho)/length(Mu_vector);
                      ...
                      for r = 1:instance_count
                         if ~(isempty(ExpData_Tensor_lib30_micro{indices_plasmids_lib24(i),indices_promoters_lib24(j),indices_rbss_lib24(k)}.Instances{r}.Pi_mumax_pmax_instance_mean))
                             ...
                             if any(isnan(Pi_predictions))
                                ...
                                rho = delta^2 * (sqrt(1 + (error./delta).^2) - 1);
                                w = sqrt(1 + log10(1 + Pi_experimental));
                                Sum_prediction_error_instance =  Sum_prediction_error_instance +  sum(w.*rho)/length(Mu_vector);
                             end % If prediction is NaN
