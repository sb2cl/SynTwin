%% Estimate_Lib24_ALL_full_model
% SynTwin workflow script: parameter estimation for Lib24 using ALL TUs
% under the complete model for RBSs (estimate both k0_sigma0 and  inv_sigma0).
%
% DESCRIPTION
%   Runs a user-defined <num_runs> of estimations using ALL constructs in Lib24.
...
%   ./Estimated_results/Results_BADS_Lib24_ALL_complete_<Use_mean>.mat
%
% DEPENDENCIES
...
% params: vector of estimated parameters of the model
    ...
    % params(4:7) = RBS_k0_sigma0, RBS strengths over sensitivities (IIC)  of (B0030, B0032,  J61100, J61101)
    % params(8:11) = RBS_inv_sigma0, RBS inverse sensitivities of (B0030, B0032,  J61100, J61101)
    % params(12) = high copy number multiplying term. So that Cn_high = x(9)*Cn_low, and we know that Cn_low=5
    ...
    x0 = [0.05,0.19, 0.16... % Starting point for omega (Use LOOCV CSIC results for non-approximated estimation)
          ...
          0.02, 0.02,  0.02, 0.02 ...   % Starting point for RBS_inv_sigma0
    ...
    lb = [0.025,0.05,0.05... %lower expected bounds for omega, promoter strengths,
          ...
          0.002, 0.002,  0.002, 0.002 ...   % lower expected boundsfor RBS_inv_sigma0
    ...
    ub = [0.15,0.35,0.35... %upper expected bounds for omega, promoter strengths,
          2.0, 10e-3,  8e-3,2e-2 ...  %upper expected bounds for RBS_k0_sigma0, 
          0.05, 0.05,  0.05, 0.05 ...   % upper expected bounds for RBS_inv_sigma0
    ...
    plb = [0.04,0.15,0.12... %Plausible lower bounds for omega, promoter strengths,
          0.5, 4e-3,  3e-3,4e-3 ...  %Plausible lower bounds bounds for RBS_k0_sigma0, 
           0.015, 0.015,  0.015, 0.015 ...   %Plausible lower bounds for RBS_inv_sigma0
    ...
    pub = [0.06,0.25,0.20... %Plausible upper bounds for omega, promoter strengths,
          ...
          0.025, 0.025,  0.025, 0.025 ...   %Plausible upper bounds for RBS_inv_sigma0
    ...
    parfor num_run=1:num_runs 
        J=@(parameters) J4_LogPI_Lib24_ALL_full_model(parameters,model_c,Use_mean,ExpData_Tensor_lib30_micro,HEM);
        [params, Jmin_value] = bads(J,x0,lb,ub,plb,pub,[],options)
         Results_BADS_Lib24_ALL_complete_v2{num_run}.results=[params, Jmin_value]; 
    end
    ...
    save(file_tensor, "Results_BADS_Lib24_ALL_complete_v2", "-mat");
