...
% Regenerate Lib6 result tensors across fixed d_mA values.
%
% DESCRIPTION
%   Loads the five B0034 estimation files and generates one complete Lib6
%   result tensor for each fixed d_mA value.
%
% FIXED VALUES
...
%
% OUTPUTS
%   Results_Tensor_Lib6_ALL_reduced_model_<Use_mean>_dma_<tag>.mat
%
%   These generated files are not distributed because of their size.
%
% USAGE
%   Generate_Results_Lib6_ALL_reduced_model_set_dma
%
...
for case_idx = 1:numel(set_dma)
    ...
    if ~isfield(S,'Results_BADS_B0034_ALL_reduced')
        error('Generate_Results_Lib6_ALL_reduced_model_set_dma:MissingVariable', ...
    ...
    for run_idx = 1:num_runs
        Matrix_tempo_All = [Matrix_tempo_All; Results_BADS_B0034_ALL_reduced{run_idx}.results]; %#ok<AGROW>
    end
    ...
    % Build the Lib6 result tensor for the current d_mA case.
            ...
            for r=1:length(indices_rbss_lib6)
              ...
              Results_Tensor_Lib6_ALL_reduced{p,q,r}.TU_color_code =  ExpData_Tensor_lib30_micro{p,q,indices_rbss_lib6(r)}.TU_color_code;
               ...
               %Plasmids p=1 high copy (pGreen), p=2-> low copy (pSC101)
                ...
                Results_Tensor_Lib6_ALL_reduced{p,q,r}.RBS_k0_sigma0_raw = Estimated_parameters.ALL_raw(:,1); 
                Results_Tensor_Lib6_ALL_reduced{p,q,r}.RBS_k0_sigma0_mean = Estimated_parameters.ALL_mean(:,1); 
                Results_Tensor_Lib6_ALL_reduced{p,q,r}.RBS_k0_sigma0_std = Estimated_parameters.ALL_std(:,1); 
    ...
    file_name = "Results_Tensor_Lib6_ALL_reduced_model_"  + Use_mean + "_dma_" + extractAfter(num2str(model_c.dm_c), '.') + ".mat";
    file_tensor = fullfile(SCRIPT_DIR, file_name);
    save(file_tensor, "Results_Tensor_Lib6_ALL_reduced", "Estimated_parameters", "-mat");
