...
% Streamlined supplementary-figure script for robustness to dmA.
% Figures generated:
%   1) Cost-function values and parameter sensitivities.
%   2) Parameter-wise violin plots across dmA.
%   3) Grouped collapsed violin plots with balanced sampling per dmA,
%      in absolute values and logarithmic y-scale, with the reference
%      distribution (dmA = 0.2) superimposed in a darker shade.
...
writetable(cost_summary, fullfile(SCRIPT_DIR, 'Table_dmA_cost_summary.csv'));
writetable(sensitivity_summary, fullfile(SCRIPT_DIR, 'Table_dmA_loglog_sensitivity.csv'));
writetable(collapsed_summary, fullfile(SCRIPT_DIR, 'Table_dmA_collapsed_grouped_balanced_summary.csv'));
...
export_figure(fig1, fullfile(SCRIPT_DIR, 'Fig_dmA_CostAndSensitivity'));
...
export_figure(fig2, fullfile(SCRIPT_DIR, 'Fig_dmA_IndividualViolins_ByParameter'));
...
export_figure(fig3, fullfile(SCRIPT_DIR, 'Fig_dmA_CollapsedViolins_Grouped_Balanced_AbsoluteLog_OverlayRef'));
