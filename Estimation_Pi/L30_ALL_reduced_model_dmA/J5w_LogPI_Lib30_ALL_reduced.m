function Sum_prediction_error = J5w_LogPI_Lib30_ALL_reduced(params,model_c,Use_mean,ExpData_Tensor_lib30_micro,HEM,RBS_inv_sigma0,delta)
%% J5w_LogPI_Lib30_ALL_reduced
% Evaluate the Lib30 reduced-model objective function.
%
% The parameter vector contains three promoter rates, five RBS intrinsic
% initiation capacities, and the pGreen/pSC101 copy-number multiplier.
% The selected d_mA value enters through model_c.dm_c.
%
% Residuals are evaluated in base-10 logarithmic synthesis-rate space with a
% weighted pseudo-Huber loss. The function is deterministic and produces no
% figures or files.
...
for i=1:length(indices_plasmids_lib30) %Plasmids i=1 high copy (pGreen), i=2-> low copy (pSC101)
    for j=1:length(indices_promoters_lib30)
        for k=1:length(indices_rbss_lib30)
            ...
            Gen_cn = model_c.N_pSC101*(i-1 + mod(i,2)*params(length(indices_promoters_lib30)+length(indices_rbss_lib30)+1) );
