function Sum_prediction_error = J5w_LogPI_Lib30_ALL_full_model(params,model_c,Use_mean,ExpData_Tensor_lib30_micro,HEM,delta)
%% J5w_LogPI_Lib30_ALL_full_model
% Evaluate the Lib30 complete-model objective function.
%
% The parameter vector contains three promoter rates, five kappa^0 values,
% five rho^0 values, and one pGreen/pSC101 copy-number multiplier.
%
% Residuals are evaluated in base-10 logarithmic synthesis-rate space and
% penalized with a weighted pseudo-Huber loss. The objective is averaged over
% growth-rate points, replicates, and all 30 constructs.
%
% The function is deterministic and produces no figures or files.

indices_plasmids_lib30 = [1,2];
indices_promoters_lib30 = [1,2,3];
indices_rbss_lib30 = [1,2,3,4,5];
num_constructs = length(indices_plasmids_lib30)*length(indices_promoters_lib30)*length(indices_rbss_lib30);
Sum_prediction_error = 0;
% NOTICE each combination (i,j,k) below will correspond to a library transcriptional unit (TU)
for i=1:length(indices_plasmids_lib30) %Plasmids i=1 high copy (pGreen), i=2-> low copy (pSC101)
    for j=1:length(indices_promoters_lib30)
        for k=1:length(indices_rbss_lib30)
            Omega = params(j);
            RBS_k0_sigma0 = params(length(indices_promoters_lib30)+k);
            RBS_inv_sigma0 = params(length(indices_promoters_lib30)+length(indices_rbss_lib30)+k);
            Gen_cn = model_c.N_pSC101*(i-1 + mod(i,2)*params(length(indices_promoters_lib30)+2*length(indices_rbss_lib30)+1) );
            if strcmp(Use_mean,'Global') 
              
            % obtain the cost index using the mean data over the instances of the construct
                Mu_vector = ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Mu_mumax_pmax_global_mean;
                Pi_experimental = ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Pi_mumax_pmax_global_mean;
             ...
             elseif strcmp(Use_mean,'Instances' ) 
                 % obtain the cost index using the means of each instance (experiment with set of 10 wells) of the construct   
                  instance_count = length(ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances);
                  Sum_prediction_error_instance=0;
                  valid_instances = 0;
                  for r = 1:instance_count
                     if ~(isempty(ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Pi_mumax_pmax_instance_mean))
                         Mu_vector = ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Mu_mumax_pmax_instance_mean;
                         Pi_experimental = ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Pi_mumax_pmax_instance_mean;
                         Pi_predictions = Get_synthesis_predictions_lite(HEM,model_c, Mu_vector,RBS_k0_sigma0,RBS_inv_sigma0,Omega,Gen_cn);
                         if any(isnan(Pi_predictions))
             ...
             % obtain the cost index using the raw data of all instances (wells x instances) of the construct
                 ...
                 valid_instances=0;
                  for r = 1:instance_count
                     if ~(isempty(ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Pi_mumax_pmax_instance_mean))
                         wells_count = length(ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Wells);
                         Sum_prediction_error_wells=0;
                         valid_wells=0;
                         for w = 1:wells_count
                             if ~isempty(ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Wells{w}.Pi_mumax_pmax)
                                 % Process the data for each well
                                 Mu_vector = ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Wells{w}.Mu_mumax_pmax;
                                 Pi_experimental = ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Wells{w}.Pi_mumax_pmax;
                                 Pi_predictions = Get_synthesis_predictions_lite(HEM,model_c, Mu_vector,RBS_k0_sigma0,RBS_inv_sigma0,Omega,Gen_cn);
                                 if any(isnan(Pi_predictions))
