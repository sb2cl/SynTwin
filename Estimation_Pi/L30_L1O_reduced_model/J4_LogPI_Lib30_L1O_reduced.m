function Sum_prediction_error = J4_LogPI_Lib30_L1O_reduced(params,Construct_2_LO,model_c,Use_mean,ExpData_Tensor_lib30_micro,HEM)
%% J4_LogPI_Lib30_L1O_reduced
...
num_constructs = length(indices_plasmids_lib30)*length(indices_promoters_lib30)*length(indices_rbss_lib30)-1;
                ...
                % obtain the cost index using the mean data over the instances of the construct
                 ...
                 elseif strcmp(Use_mean,'Instances' ) 
                     % obtain the cost index using the means of each instance (experiment with set of 10 wells) of the construct   
                      ...
                      Sum_prediction_error =  Sum_prediction_error/instance_count; %averaged Sum_prediction_error over number of instances of the current TU
                 else 
                     ...
                     instance_count = length(ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances);
                      for r = 1:instance_count
                         if ~(isempty(ExpData_Tensor_lib30_micro{indices_plasmids_lib30(i),indices_promoters_lib30(j),indices_rbss_lib30(k)}.Instances{r}.Pi_mumax_pmax_instance_mean))
                             ...
                             Sum_prediction_error =  Sum_prediction_error/wells_count; %averaged Sum_prediction_error over number of wells of the current instance of current TU
                          end %If instance has data
                      end % for each instance of the current TU
                      Sum_prediction_error =  Sum_prediction_error/instance_count; %averaged Sum_prediction_error over number of instances of the current TU
