%% Estimate_Lib30_L1O_reduced_model
% SynTwin workflow script: parameter estimation for Lib30 using L1O cross-validation
...
%   ./Estimated_results/Results_BADS_Lib30_L1O_reduced_<Use_mean>.mat
...
%   - Uses: J4_LogPI_Lib30_L1O_reduced (objective function)
...
for i=1:length(indices_plasmids_lib30) %Plasmids i=1 high copy (pGreen), i=2-> low copy (pSC101)
    for j=1:length(indices_promoters_lib30)
        for k=1:length(indices_rbss_lib30)
             %   Results_BADS_Lib30_LOOCV_approx = NaN*ones(num_runs,10);
                ...
                % params: vector of estimated parameters of the model
                    ...
                    % params(4:8) = RBS_k0_sigma0, RBS strengths over sensitivities of (B0030, B0032, B0034, J61100, J61101)
                ...
                parfor num_run=1:num_runs 
                    J=@(parameters) J4_LogPI_Lib30_L1O_reduced(parameters,Construct_2_LO,model_c,Use_mean,ExpData_Tensor_lib30_micro,HEM);
                    [params, Jmin_value] = bads(J,x0,lb,ub,plb,pub,[],options)
                     Results_BADS_Lib30_LOOCV_approx{num_run}.results=[params, Jmin_value]; 
                end
                ...
                file_name = "Results_BADS_Lib30_L1O_reduced_" + ...
                                num2str(indices_plasmids_lib30(Construct_2_LO(1))) + ...
                                num2str(indices_promoters_lib30(Construct_2_LO(2))) + ...
                                num2str(indices_rbss_lib30(Construct_2_LO(3))) + "_" + ...
                                Use_mean + ".mat";
                file_tensor = fullfile(results_dir, file_name);
                save(file_tensor, "Results_BADS_Lib30_LOOCV_approx", "-mat");
        end %RBS
    end %PROMOTER
end %PLASMID
